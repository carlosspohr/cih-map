...
function CIHMap()
{
	...
	this.init = function()
	{
		...
		try
		{
			...
			$('#' + this.uiContentDiv).addClass('cih-map-content-div').html(this.createDivMapContent());
	        ...
	        var instancia = this.profileFactory();
	        instancia.start();
		} catch (e) {
			$('#' + this.uiBodyDiv).remove();
			$('<div id="ddesm__"></div>')
				.attr('title', 'Erro de carregamento')
				.html("<p class='error_description'>Erro no carregamento do mapa. <br/>Causa: <span>" + e + "</span></p>")
				.dialog({
					width: 		300,
					height: 	150,
					modal: 		true,
	...
	this.createLonLatInputs = function()
	{
		...
		inputs += "		<td width='20%'><label class='label-map'>" + this.options.mapInformationProjecao + ":<br/></label><div id='precision_box__'>EGSP:4326</div></td>";
		inputs += "</tr></table>";
	...
	this.addDefaultMapControls = function ()
	{
		...
		this.getMap().addControl (panZoom);
	...
	this.createSearchLocationForm = function()
	{
		...
		inputs += "<td width='15%'>" +
				...
				"		</select>" +
				"</td></tr>";
		inputs += "<tr><td colspan='4'><div id='result-consulta-localizacao'></div></td></tr></table>";
	...
	this.createCloseButton = function()
	{
		var inputs = "";
		inputs += "<table><tr>";
		inputs += "		<td style='text-align:right;'><input type='button' class='submit' id='button-close-dialog' value='" + this.options.dialogButtonClose + "'/></td>";
		inputs += "</tr></table>";
	...
	{
		var div = "	<div id='" + this.mapDivId + "' style='width:100%; height:290px; margin:0px; padding:0px;'>&nbsp;</div>";
	...
	this.customCommonsButtonsActionsAfterClick = function(action)
	{
		console.log('action: ' + action);
		if(action == 'navigation')
		{
			$("#btOlControlNavigation")			.attr('disabled', true);
			$("#btOlControlAddGeometryString")	.attr('disabled', false);
			$("#btOlControlDeleteGeometryString").attr('disabled', false);
		}
		if(action == 'add')
		{
			$("#btOlControlNavigation")			.attr('disabled', false);
			$("#btOlControlAddGeometryString")	.attr('disabled', true);
			$("#btOlControlDeleteGeometryString").attr('disabled', false);
		}
		if(action == 'delete')
		{
			$("#btOlControlNavigation")			.attr('disabled', false);
			$("#btOlControlAddGeometryString")	.attr('disabled', false);
			$("#btOlControlDeleteGeometryString").attr('disabled', true);
		}
	};
}